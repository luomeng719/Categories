...
static NSString * kILSOpenUDIDSessionCache = nil;
static NSString * const kILSOpenUDIDKey = @"OpenUDID";
static NSString * const kILSOpenUDIDSlotKey = @"OpenUDID_slot";
static NSString * const kILSOpenUDIDAppUIDKey = @"OpenUDID_appUID";
static NSString * const kILSOpenUDIDTSKey = @"OpenUDID_createdTS";
static NSString * const kILSOpenUDIDOOTSKey = @"OpenUDID_optOutTS";
static NSString * const kILSOpenUDIDDomain = @"org.OpenUDID";
static NSString * const kILSOpenUDIDSlotPBPrefix = @"org.OpenUDID.slot.";
static int const kILSOpenUDIDRedundancySlots = 100;
...
+ (void) _setDict:(id)dict forPasteboard:(id)pboard {
#if TARGET_OS_IPHONE || TARGET_IPHONE_SIMULATOR		
    [pboard setData:[NSKeyedArchiver archivedDataWithRootObject:dict] forPasteboardType:kILSOpenUDIDDomain];
#else
    [pboard setData:[NSKeyedArchiver archivedDataWithRootObject:dict] forType:kILSOpenUDIDDomain];
#endif
}
...
+ (NSMutableDictionary*) _getDictFromPasteboard:(id)pboard {
#if TARGET_OS_IPHONE || TARGET_IPHONE_SIMULATOR	
    id item = [pboard dataForPasteboardType:kILSOpenUDIDDomain];
#else
	id item = [pboard dataForType:kILSOpenUDIDDomain];
#endif	
    if (item) {
        @try{
...
+ (NSString*) valueWithError:(NSError **)error {
    ...
    @synchronized (self) {
        ...
        if (kILSOpenUDIDSessionCache!=nil) {
            if (error!=nil)
                *error = [NSError errorWithDomain:kILSOpenUDIDDomain
                                             code:kILSOpenUDIDErrorNone
                                         userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"OpenUDID in cache from first call",@"description", nil]];
            return kILSOpenUDIDSessionCache;
        }
        ...
        //
        NSString * appUID = [defaults objectForKey:kILSOpenUDIDAppUIDKey];
        if(appUID == nil)
        {
        ...
        //
        id localDict = [defaults objectForKey:kILSOpenUDIDKey];
        if ([localDict isKindOfClass:[NSDictionary class]]) {
            ...
            openUDID = [localDict objectForKey:kILSOpenUDIDKey];
            myRedundancySlotPBid = [localDict objectForKey:kILSOpenUDIDSlotKey];
            optedOutDate = [localDict objectForKey:kILSOpenUDIDOOTSKey];
        ...
        NSMutableDictionary* frequencyDict = [NSMutableDictionary dictionaryWithCapacity:kILSOpenUDIDRedundancySlots];
        for (int n=0; n<kILSOpenUDIDRedundancySlots; n++) {
            NSString* slotPBid = [NSString stringWithFormat:@"%@%d",kILSOpenUDIDSlotPBPrefix,n];
#if TARGET_OS_IPHONE || TARGET_IPHONE_SIMULATOR
            ...
            if (slotPB==nil) {
                ...
                NSString* oudid = [dict objectForKey:kILSOpenUDIDKey];
                ...
                NSString* gid = [dict objectForKey:kILSOpenUDIDAppUIDKey];
                if (gid!=nil && [gid isEqualToString:appUID]) {
                    ...
                    if (optedOut) {
                        optedOutDate = [dict objectForKey:kILSOpenUDIDOOTSKey];
                        optedOut = optedOutDate!=nil;
                    }
                }
            }
        ...
        if (openUDID==nil) {
            ...
            if (localDict==nil) {
                ...
                [localDict setObject:openUDID forKey:kILSOpenUDIDKey];
                [localDict setObject:appUID forKey:kILSOpenUDIDAppUIDKey];
                [localDict setObject:[NSDate date] forKey:kILSOpenUDIDTSKey];
                if (optedOut) [localDict setObject:optedOutDate forKey:kILSOpenUDIDTSKey];
        ...
        if (availableSlotPBid!=nil && (myRedundancySlotPBid==nil || [availableSlotPBid isEqualToString:myRedundancySlotPBid])) {
            ...
            //
            if (localDict) {
                [localDict setObject:availableSlotPBid forKey:kILSOpenUDIDSlotKey];
                saveLocalDictToDefaults = YES;
            }
        ...
        if (localDict && saveLocalDictToDefaults)
            [defaults setObject:localDict forKey:kILSOpenUDIDKey];
        ...
        //
        if (optedOut) {
            if (error!=nil) *error = [NSError errorWithDomain:kILSOpenUDIDDomain
                                                         code:kILSOpenUDIDErrorOptedOut
            ...
            kILSOpenUDIDSessionCache = [[NSString stringWithFormat:@"%040x",0] retain];
            return kILSOpenUDIDSessionCache;
        }
        ...
        if (error!=nil) {
            if (isCompromised)
                *error = [NSError errorWithDomain:kILSOpenUDIDDomain
                                             code:kILSOpenUDIDErrorCompromised
        ...
        kILSOpenUDIDSessionCache = [openUDID retain];
        return kILSOpenUDIDSessionCache;
    }
}
...
+ (void) setOptOut:(BOOL)optOutValue {
    ...
    id dict = [defaults objectForKey:kILSOpenUDIDKey];
    ...
    if (optOutValue)
        [dict setObject:[NSDate date] forKey:kILSOpenUDIDOOTSKey];
    else
        [dict removeObjectForKey:kILSOpenUDIDOOTSKey];

  	// store the dictionary locally
    [defaults setObject:dict forKey:kILSOpenUDIDKey];
    
    OpenUDIDLog(@"Local dict after opt-out = %@",dict);
    
    // reset memory cache 
    kILSOpenUDIDSessionCache = nil;
